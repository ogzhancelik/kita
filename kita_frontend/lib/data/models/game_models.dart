...
class KitaGameEngine {
  ...
  /// AI heuristic evaluation with difficulty levels
  KitaMove? getBestAiMove({int difficulty = 1}) {
    final legalMoves = getLegalMoves();
    if (legalMoves.isEmpty) return null;

    // Easy (random move)
    if (difficulty == 0) {
      final shuffled = List<KitaMove>.from(legalMoves)..shuffle();
      return shuffled.first;
    }

    // Intermediate & Hard: Minimax with alpha-beta heuristic
    KitaMove? bestMove;
    int bestScore = -999999;

    for (final move in legalMoves) {
      final sim = applyMove(move);

      // Score move
      int score = _evaluateState(sim, turn);

      // Look 1-step ahead for hard
      if (difficulty >= 2 && !sim.isGameOver) {
        final oppMoves = sim.getLegalMoves();
        int worstOpponentThreat = 999999;
        for (final oppMove in oppMoves) {
          final oppSim = sim.applyMove(oppMove);
          final oppScore = _evaluateState(oppSim, turn);
          if (oppScore < worstOpponentThreat) {
            worstOpponentThreat = oppScore;
          }
        }
        score = worstOpponentThreat;
      }
  ...
  int _evaluateState(KitaGameEngine g, PieceTeam myTeam) {
    final status = g.getStatus();
    if (status == GameStatus.draw) return 0;
    if (status == GameStatus.whiteWins) {
      return myTeam == PieceTeam.white ? 100000 : -100000;
    }
    if (status == GameStatus.blackWins) {
      return myTeam == PieceTeam.black ? 100000 : -100000;
    }

    int score = 0;
    final myKingId = myTeam == PieceTeam.white ? 'WK' : 'BK';
    final oppKingId = myTeam == PieceTeam.white ? 'BK' : 'WK';

    // King safety and presence
    if (g.positions[myKingId] != null) score += 5000;
    if (g.positions[oppKingId] == null) score += 20000;

    // Mobility
    final myMoves = g.getLegalMoves();
    score += myMoves.length * 15;

    // Control of high-value tiles (2 and 3)
    for (final entry in g.positions.entries) {
      if (entry.value == null) continue;
      final piece = KitaPiece.allPieces[entry.key];
      final val = KitaBoardConfig.tileValue(entry.value!);
